...
# VType is a type alias defined in _analysis_py.py
...
def vtype_from_descriptor(fd: FieldDescriptor):
...
cdef object _descriptor_to_component_string(fd: FieldDescriptor):
...
def merge_vtypes(a: VType, b: VType, resolver: ClassResolver | None = None):
...
class FrameState:
    ...
    def set_local(self, index: int, vtype: VType):
...
def initial_frame(method: MethodModel, class_name: str):
...
cdef object _merge_frames(a: FrameState, b: FrameState, resolver: ClassResolver | None):
...
cdef object _is_terminal(insn: InsnInfo):
...
cdef object _is_branch_or_switch(insn: InsnInfo):
...
def build_cfg(code: CodeModel):
    ...
    if pending_labels and blocks:
        for lbl in pending_labels:
            label_to_block_map[lbl] = blocks[len(blocks) - 1]
...
cdef object _find_first_insn(items: list[CodeItem]):
...
cdef object _find_next_insn(items: list[CodeItem], start: int):
...
cdef bint _instruction_may_throw(insn: InsnInfo) except -1:
...
cdef object _propagate_exception_handlers(
...
cdef object _simulate_raw_insn(insn: InsnInfo, state: FrameState):
...
cdef object _replace_uninitialized(
    state: FrameState,
...
cdef object _prepare_analysis_code(code: CodeModel):
...
cdef object _find_label_for_insn(code: CodeModel, target_insn: InsnInfo):
...
cdef int _verification_type_info_size(vti: VerificationTypeInfo) except -1:
...
cdef int _stack_map_frame_size(frame: StackMapFrameInfo) except -1:
...
cdef int _stack_map_table_attribute_length(frames: Sequence[StackMapFrameInfo]) except -1:
