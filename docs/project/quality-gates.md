...
The test suite has unit-level coverage across all modules, including per-attribute-type parsing, all instruction operand shapes, constant-pool edge cases, Modified UTF-8 behavior, descriptor validation, constant-pool builder safety, binary writer primitives, label/layout lowering, symbolic operand wrapper lifting/lowering, hierarchy resolution, control-flow simulation, CFG oracle comparisons, frame recomputation, structural validation diagnostics, byte-for-byte `ClassWriter.write()` roundtrips across the compiled Java fixture corpus, byte-for-byte `ClassModel.to_bytes()` roundtrips across the same corpus with preserved nested `Code`-attribute ordering, and multi-release four-tier validation tests across the compiled Java fixture corpus at `--release 8, 11, 17, 21, 25`, filtered by each fixture's minimum supported release.

All four tiers of the bytecode validation framework are now implemented ([#14](https://github.com/smithtrenton/pytecode/issues/14)). `tests/test_validation.py` runs byte-for-byte roundtrip (Tier 1), structural verification via `verify_classfile()` and `javap` (Tier 2), CP-aware semantic diff via the `javap_parser.py` engine (Tier 3), and JVM loading with `-Xverify:all` via the `VerifierHarness.java` classloader (Tier 4) across all (fixture, release) combinations. The constant pool strategy uses preserve-on-roundtrip as the default mode with deterministic insertion order for v1 from-scratch generation and opt-in javac-compatible ordering reserved for later compatibility work.

The remaining work is now centered on higher-level mutation workflows, archive support, and documentation coverage now that emission, validation, and frame recomputation are in place. The roadmap continues with:
...
- optional JAR rewrite support for transformed archives ([#15](https://github.com/smithtrenton/pytecode/issues/15))
