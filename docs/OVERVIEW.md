...
- Composable transform pipelines and matcher DSL ([#6](https://github.com/smithtrenton/pytecode/issues/6), [#20](https://github.com/smithtrenton/pytecode/issues/20), [#21](https://github.com/smithtrenton/pytecode/issues/21)): `pytecode.transforms` now provides callable `Pipeline` objects, `Matcher` predicates with `&` / `|` / `~` composition, `on_classes()` / `on_fields()` / `on_methods()` / `on_code()` lifting helpers, owner-class filtering for field/method/code lifting, regex helpers, lightweight structural helpers for direct superclasses/interfaces, class versions, special method names, and return descriptors, plus access-flag convenience matchers. The `FieldTransform`, `MethodTransform`, and `CodeTransform` protocols pass owning context (`ClassModel` for field/method transforms; `MethodModel` and `ClassModel` for code transforms) so transforms can inspect where they are in the class hierarchy. Pipelines remain ordinary class transforms, so they plug directly into `JarFile.rewrite(transform=...)`.
