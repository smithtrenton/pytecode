...
- Multi-release four-tier validation coverage ([#14](https://github.com/smithtrenton/pytecode/issues/14)): `tests/test_validation.py` parametrizes 13 dedicated Java fixtures (covering enums, generics, lambdas, records, sealed classes, pattern matching, and Java 25 features) across `--release 8, 11, 17, 21, 25`. Each (fixture, release) pair runs all four tiers: byte-for-byte roundtrip (T1), `verify_classfile()` + `javap` structural check (T2), `javap_parser.py` CP-aware semantic diff (T3), and JVM loading via `VerifierHarness.java` with `-Xverify:all` (T4).
